...
export const sendMessage = async (
  ...
  if (platform === 'discord') {
    try {
      const { discordClient } = await import('./discord.service');
      const user = await discordClient.users.fetch(platformId);
      if (user) {
        await user.send(text);
        console.log(`✅ Discord DM message delivered: userId=${platformId}`);
        return true;
      }
    } catch (error: any) {
      console.error(
